...
st.markdown("""
    ...
    /* Center the tabs to make it look like an App */
    .stTabs [data-baseweb="tab-list"] {
        justify-content: center;
    }
    /* Style the images to have rounded corners */
    img {
        border-radius: 15px;
...
st.markdown('<div class="sub-title">Upload a photo or use your camera to detect emotional states.</div>', unsafe_allow_html=True)
...
def run_analysis(image_file):
    with st.spinner("Analyzing facial features..."):
        ...
        if len(faces) == 0:
            ...
            # Display Processed Image and Metrics side-by-side
...
# --- TABBED INTERFACE ---
tab1, tab2 = st.tabs(["📸 Take a Picture", "🖼️ Upload Image"])
...
with tab2:
    st.write("Upload a clear photo of a face for analysis.")
    uploaded_img = st.file_uploader("Drag and drop an image here", type=["jpg", "png", "jpeg"], label_visibility="collapsed")
    if uploaded_img is not None:
        run_analysis(uploaded_img)
